...
Sub 生成版本号()
    ...
    ModuleFileName = FilePath & "\模块1源代码-" & Version & "-" & Format(RiviseDate, "YYYYMMDD") & ".bas"
...
Sub 指标点数据表公式()
    ...
    Range("D6").Select
    ActiveCell.FormulaR1C1 = _
        "=IF(OR(R4C3="""",VLOOKUP(R4C3,INDIRECT(""'""&INDEX(专业矩阵状态!C3,MATCH('2-课程目标和综合分析（填写）'!R7C2,专业矩阵状态!C2,))&""'!$B$3:$AS$3""),ROW(RC3)-2,0)=""""),"""",VLOOKUP(R4C3,INDIRECT(""'""&INDEX(专业矩阵状态!C3,MATCH('2-课程目标和综合分析（填写）'!R7C2,专业矩阵状态!C2,))&""'!$B$3:$AS$3""),ROW(RC3)-2,0))"
    Range("E6").Select
    ActiveCell.FormulaR1C1 = _
        "=IF(OR(R2C2="""",RC[-1]=""""),"""",IF(ISNA(VLOOKUP(R4C[-2]&""-""&R3C2&""*"",INDIRECT(""'""&INDEX(专业矩阵状态!C3,MATCH('2-课程目标和综合分析（填写）'!R7C2,专业矩阵状态!C2,))&""'!$A:$AS""),ROW(RC1)-1,0)),0,VLOOKUP(R4C[-2]&""-""&R3C2&""*"",INDIRECT(""'""&INDEX(专业矩阵状态!C3,MATCH('2-课程目标和综合分析（填写）'!R7C2,专业矩阵状态!C2,))&""'!$A:$AS""),ROW(RC1)-1,0)))"
    Range("F6").Select
...
Public Sub Excel2PDF(WorkSheetName As String, PathName As String, FileName As String)
    ...
    End If
    ActiveSheet.ExportAsFixedFormat Type:=xlTypePDF, FileName:=PDFFileName, Quality:=xlQualityStandard, IncludeDocProperties:=True, IgnorePrintAreas:=False, OpenAfterPublish:=True
    If (WorkSheetVisble = False) Then
...
' 保存文件 宏
    ...
    Application.ScreenUpdating = False
    MsgBox ("正在进行工作表格式调整，并保存文件，请耐心等待！")
    Worksheets("2-课程目标和综合分析（填写）").Activate
    Call 设置教学过程登记表
    Call 设置质量分析报告格式
...
Sub 导入教学任务()
    ...
    If MyFile.FileExists(FileName) = False Then
      MsgBox ("数据源-教学任务.xls不存在，请将数据源-教学任务.xls复制到当前文件夹")
...
Sub 导入毕业要求矩阵(Major As String)
    ...
    FileName = FileName & "\" & SourceWorkBook
    If MyFile.FileExists(FileName) = False Then
      MsgBox ("数据源-" & Major & "-指标点数据矩阵.xls不存在，请将数据源-" & Major & "-指标点数据矩阵.xls复制到当前文件夹")
      FileName = Application.GetOpenFilename("Microsoft Excel Files (*.xls;*.xlsx;*.xlsm),*.xls;*.xlsx;*.xlsm", , "Get list")
    '调用Windows打开文件对话框
    If (FileName = False) Then Exit Sub
    SourceWorkBook = Mid(FileName, InStrRev(FileName, "\") + 1)
...
Sub 导入成绩表()
    ...
    If (Range("Q3").Value = "认证未提交成绩") Then
        MsgBox ("【课程目标和综合分析工作表认证状态为未提交成绩，不需要导入成绩表】")
    ...
    End If
    If MyFile.FileExists(FileName) = False Then
        MsgBox ("成绩表-" & CourseNumber & ".xls" & "不存在，请手动指定")
...
Sub 导入教务系统成绩表()
    ...
    End If
    If MyFile.FileExists(FileName) = False Then
        MsgBox (CourseNumber & "期末成绩.xls" & "不存在，请手动指定")
...
Sub 导入实验成绩表()
    ...
    If MyFile.FileExists(FileName) = False Then
        MsgBox ("实验成绩表-" & CourseNumber & ".xls" & "不存在，请手动指定")
        FileName = Application.GetOpenFilename("Microsoft Excel Files (*.xls;*.xlsx;*.xlsm),*.xls;*.xlsx;*.xlsm", , "Get list")
        '调用Windows打开文件对话框
        If (FileName = False) Then Exit Sub
        SourceWorkBook = Mid(FileName, InStrRev(FileName, "\") + 1)
...
Function 导入教学过程登记表() As Boolean
    ...
    Else
        ...
        If MyFile.FileExists(FileName) = False Then
            MsgBox (FileName & "不存在，请手动指定" & Xueqi & "学期的教学过程登记表，支持导入学分管理系统下载成绩表名单，也可以从教务系统复制教学过程登记表名单到新EXCEL文档")
            FileName = Application.GetOpenFilename("Microsoft Excel Files (*.xls;*.xlsx;*.xlsm),*.xls;*.xlsx;*.xlsm", , "Get list")
        ...
        If (Range("$T$1").Value = "无名单") Then
            MsgBox ("导入的教学过程登记表中没有该课号的名单，请手动指定教学过程登记表文件，支持导入学分管理系统下载成绩表名单，也可以从教务系统复制教学过程登记表名单到新EXCEL文档")
            FileName = Application.GetOpenFilename("Microsoft Excel Files (*.xls;*.xlsx;*.xlsm),*.xls;*.xlsx;*.xlsm", , "Get list")
            '调用Windows打开文件对话框
            If (FileName = False) Then
                Application.EnableEvents = True
